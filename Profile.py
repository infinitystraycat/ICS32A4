...
should catch in your own code.
It is raised when attempting to deserialize a dsu file
to a Profile object.
"""
...
class Post(dict):
    ...
    upon instantiation and when the entry object is set and an entry
...
class Profile:
    ...
    provided by each new user created within your program for a2.
    Pay close attention to the properties and functions in this class as
    you will need to make use of each of them in your program.
    ...
    So if multiple Posts objects are created,
    ...
    operation on the posts returned from the get_posts
    ...
    def save_profile(self, path: str) -> None:
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                ...
                raise DsuFileError("Error while attempting to process "
                                   "the DSU file.", ex)
        else:
